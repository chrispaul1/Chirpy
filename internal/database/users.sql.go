...
const upgradeUserChirpyMembership = `-- name: UpgradeUserChirpyMembership :one
UPDATE users
...
func (q *Queries) UpgradeUserChirpyMembership(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, upgradeUserChirpyMembership, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
